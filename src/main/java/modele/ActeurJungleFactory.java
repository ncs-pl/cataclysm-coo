...
public class ActeurJungleFactory implements ActeurAbstractFactory {
    ...
    @Override public Objet      creerObjetChampignonDrogue(int ligne, int colonne, int maxLignes, int maxColonnes) { return new ChampignonHallucinogene(ligne, colonne, maxLignes, maxColonnes); } // TODO(nico): importer le champignon quand Younes l'aura crée.
    @Override public Predateur  creerPredateur1(int ligne, int colonne, int maxLignes, int maxColonnes)            { return new Serpent(ligne, colonne, maxLignes, maxColonnes);                 } // TODO(nico): importer le serpent quand Younes l'aura crée.
    @Override public Predateur  creerPredateur2(int ligne, int colonne, int maxLignes, int maxColonnes)            { return new Scorpion(ligne, colonne, maxLignes, maxColonnes);                } // TODO(nico): importer le scorpion quand Younes l'aura crée.
